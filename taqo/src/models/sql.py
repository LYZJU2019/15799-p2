...
class SQLModel(QTFModel):
    ...
    def create_tables(self, conn, skip_analyze=False, db_prefix=None):
        ...
        if DDLStep.DROP in self.config.ddls:
            ...
            teardown_queries.insert(0, "-- DROP QUERIES")    # inserting comments
        ...
        if DDLStep.ANALYZE in self.config.ddls:
            _, analyze_queries = self.evaluate_ddl_queries(conn, DDLStep.ANALYZE,
                                                                         db_prefix)
    ...
    def import_from_local(self, cur, cleaned):
        # e.g. COPY ts2 FROM '$DATA_PATH/ts2.csv' with (delimiter ',', FORMAT csv, NULL 'NULL');
        # NOTE: $DATA_PATH has been replaced by the actual path in apply_variables before calling this function

        copy_re = r"(?i)\bCOPY\b\s(.+)\s\bFROM\b\s\'(.*)\'\s\bWITH\b\s\((.*\,?)\)"
        parse_re = re.findall(copy_re, cleaned, re.MULTILINE)[0]
        table_name = parse_re[0]   # ts2
        local_path = parse_re[1]   # '$DATA_PATH/ts2.csv'
        params = parse_re[2]       # "delimiter ',', FORMAT csv, NULL 'NULL'"
    ...
    """
    Example sql comments with hints:
    -- accept: a b c (only consider join order a -> b -> c)
    -- reject: NestLoop
    -- max_timeout: 5s
