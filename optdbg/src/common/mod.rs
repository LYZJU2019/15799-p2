...
fn format_plan_with_2preorder_help<T1: std::fmt::Display, T2: std::fmt::Display>(
	...
	writeln!(f, "{} ({}, {})", plan.name(), preorder_data1[*index], preorder_data2[*index])?;
	*index += 1;
	for child in plan.children() {
		format_plan_with_2preorder_help(f, child.clone(), indent_level + 1,
										preorder_data1, preorder_data2, index)?;
	}
	Ok(())
}
...
impl std::fmt::Display for Plan {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		format_plan_with_2preorder_help(
			f, self.tree.clone(), 0, &self.est_costs, &self.est_cards, &mut i
		)
	}
}
